<x-app-layout>
		...
		<div class="py-12">
			<form method="post" action="{{ route('location.update' , $location->id) }}">@csrf
				@method('PUT')
				<!-- Car inf -->
										...
										<div class="col-span-6 sm:col-span-6 lg:col-span-2">
											...
											<select id="voiture_id" name="voiture_id" autocomplete="country" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
												@foreach($voitures as $voiture)
													@if ($location->voiture_id == $voiture->id)
													<option value="{{ $voiture->id }}" selected>{{ $voiture->titre }}</option>
													@else
										...
										<div class="col-span-6 sm:col-span-6 lg:col-span-2">
									 	...
									 	<div class="grid grid-cols-4 gap-4 mt-4">
										  <div class="col-span-6 sm:col-span-6 lg:col-span-2">
											...
											<input type="datetime-local" name="from" id="from" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @if($errors->has('from')) is-invalid @endif" placeholder="2017-06-01T08:30" value="{{old('from', $location->from)}}">
											@if($errors->has('from'))
										  ...
										  <div class="col-span-6 sm:col-span-6 lg:col-span-2">
											...
											<input type="datetime-local" name="to" id="to" autocomplete="family-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @if($errors->has('to')) is-invalid @endif" placeholder="2017-06-02T08:30" value="{{old('to', $location->to)}}">
											@if($errors->has('to'))
												<div class="invalid-feedback">
													{{$errors->first('to')}}
												</div>
