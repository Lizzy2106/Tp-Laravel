<x-app-layout>
		...
		<div class="py-12">
			<form method="post" action="{{ route('voiture.update' , $voiture->id) }}" enctype="multipart/form-data">@csrf
				@method('PUT')
				<!-- Car inf -->
								...
								<div class="px-4 py-5 bg-white sm:p-6">
									<div class="grid grid-cols-6 gap-6">
									  <div class="col-span-6 sm:col-span-6 lg:col-span-2">
										...
										<input type="text" name="titre" id="titre" autocomplete="given-name"  value="{{old('titre', $voiture->titre)}}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @if($errors->has('titre')) is-invalid @endif">
										@if($errors->has('titre'))
									  ...
									  <div class="col-span-6 sm:col-span-3 lg:col-span-2">
										...
										<input type="text" name="marque" id="marque" autocomplete="family-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @if($errors->has('marque')) is-invalid @endif" value="{{old('marque', $voiture->marque)}}">
									  ...
									  <div class="col-span-6 sm:col-span-3 lg:col-span-2">
											...
											<input type="number" name="prix" id="prix" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @if($errors->has('prix')) is-invalid @endif" value="{{old('prix', $voiture->prix) ?? 10000}}" min="10000" step="500">
											@if($errors->has('prix'))
												<div class="invalid-feedback">
													{{$errors->first('prix')}}
												</div>
									...
									<div>
										...
										<div class="mt-1">
											<textarea id="description" name="description" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border-gray-300 rounded-md @if($errors->has('description')) is-invalid @endif" placeholder="Cette voiture est parfaite pour les familles nombreuses." value="{{old('description', $voiture->description)}}">{{old('description', $voiture->description)}}</textarea>
									...
									<div class="grid grid-cols-3 gap-3">
										...
										<div class="mt-1 flex items-center">
											<span class="inline-block h-12 w-12 rounded-full overflow-hidden bg-gray-100">
												...
												<img class="h-20 w-20" src="{{$voiture->image1}}" alt="" title="galerie">
											</span>
											...
											<div class="space-y-1 text-center">
												...
												<div class="flex text-sm text-gray-600">
													<label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
														...
														<input id="file-upload" name="image1" type="file" class="sr-only" value="{{$voiture->image1}}">
													</label>
											...
											<div class="flex items-start">
												<div class="px-4 py-5 bg-white sm:p-6">
													<div class="grid grid-cols-6 gap-6">
														...
														<div class="col-span-6 sm:col-span-6 lg:col-span-2">
															<label for="{{$agrement->titre}}" class="block text-sm font-medium text-gray-900">@if($voiture->agrements()->where('agrement_id', $agrement->id)->exists())
																<input name="agrements[]" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" id="{{$agrement->id}}" value="{{$agrement->id}}" checked>
																@else
																<input name="agrements[]" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" id="{{$agrement->id}}" value="{{$agrement->id}}">
																@endif
															{{ucfirst($agrement->titre)}}
															</label>

															@if($agrement->option1 == 'valeur')
															<input type="number" name="{{$agrement->id}}" id="{{$agrement->id}}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="2">
															@else
															<select id="{{$agrement->id}}" name="{{$agrement->id}}" autocomplete="country" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
																...
																@else
																	...
																	@if($agrement->{'option'.$i} != null)
																		<option value="{{ ucfirst($agrement->{'option'.$i}) }}">{{ ucfirst($agrement->{'option'.$i}) }}</option>
																		<!-- <option>Canada</option>
																		<option>Mexico</option> -->
																	@endif
																	@endfor
